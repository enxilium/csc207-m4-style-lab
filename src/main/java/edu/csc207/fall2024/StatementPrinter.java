...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            result += String.format("  %s: %s (%s seats)%n", play.getName(), frmt.format(thisAmount / rate),
                    p.getAudience());
            totalAmount += thisAmount;
        }
        result += String.format("Amount owed is %s%n", frmt.format(totalAmount / rate));
    ...
    private static int getAmount(Performance performance, Play play) {
        int thisAmount = 0;
        final int startTragedy = 40000;
        final int rate = 1000;
        final int rate2 = 30;

        switch (play.getType()) {
            case "tragedy":
                thisAmount = startTragedy;
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    thisAmount += rate * (performance.getAudience() - rate2);
                }
                break;
            case "comedy":
